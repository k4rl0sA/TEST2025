 ...
 */
 ...
 $c[]=new cmp('docu_confirm','h','999999999999999999','',$w.' AGe '.$o,'Documento Confirmado  del Usuario','docu_confirm',null,null,false,false,'','col-2');
  $c[]=new cmp('perfil_gest','h',3,'',$w.' AGe '.$o,'Perfil que Gestiona','perfil_gest',null,'',false,false,'','col-2',"selectDepend('perfil_gest','usuario_gest','lib.php');");
 $c[]=new cmp('usuario_gest','h','10','',$w.' AGe '.$o,'Usuario que Gestiona','usuario_gest',null,null,false,false,'','col-2');
...
LEFT JOIN eac_ruteo er ON g.idruteo=er.id_ruteo 
	...
	WHERE idruteo=$id[0] and (estado_agenda=1 or estado_agenda=6 or estado_agenda=9 or estado_agenda=11 or estado_agenda=12) and estado_llamada=1 and u.perfil IN ('AUXHOG','ADM');";
    $info = datos_mysql($sql);
	// var_dump($sql);
...
if ($a=='rute' && $b=='acciones'){
		...
		if (EnabFin($c['ACCIONES'])) {
			$rta .= acceso('seguiremot') ? "<li title='Seguimiento remoto' onclick=\"mostrar('seguiremoto','pro',event,'','lib.php',7,'Seguimiento Remoto');\"><i class='fa-solid fa-house-laptop ico' id='{$c['ACCIONES']}'></i></li>" : "";
		}
