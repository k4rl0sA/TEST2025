...
// var_dump($d);
 ...
 $c[]=new cmp('documento','t','18',$d['documento'],$w.' '.$o,'NÚMERO DE DOCUMENTO','documento',null,null,false,false,'','col-2');
...
function gra_rute(){
	...
	$params = [
		['type' => 'i', 'value' => $id[0]],
		param_null($_POST['fecha_llamada']??'','s'),
		param_null($_POST['estado_llamada']??'','i'),
		param_null($_POST['observacion']??'','s'),
		param_null($_POST['estado_agenda']??'','i'),
		param_null($_POST['motivo_estado']??'','i'),
		param_null($_POST['fecha_gestion']??'','s'),
		param_null($_POST['docu_confirm']??'','i'),
		param_null($_POST['usuario_gest']??'','i'),
		param_null($_POST['direccion_nueva_v']??'','s'),
		param_null($_POST['sector_catastral_v']??'','i'),
		param_null($_POST['nummanzana_v']??'','i'),
		param_null($_POST['predio_num_v']??'','i'),
		['type' => 's', 'value' => $bina],
		['type' => 's', 'value' => $usu]
		];
