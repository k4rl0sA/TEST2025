...
function lis_seguiremoto(){
  ...
  $pag=(isset($_POST['pag-seguiremoto']))? ($_POST['pag-seguiremoto']-1)* $regxPag:0;
	...
	$sql="SELECT `id_ruteoremoto` ACCIONES,id_ruteoremoto  'Cod Registro',R.documento ,R.nombres,fecha_seg Fecha,numsegui 'N° Seguimiento',FN_CATALOGODESC(73,estado_s) estado,FN_CATALOGODESC(170,continua_seguimiento) Cierra,U1.nombre 'Asignado 1',U2.nombre 'Asignado 2'
  FROM ruteo_remoto A 
	LEFT JOIN eac_ruteo R ON A.idruteoclas=R.id_ruteo
  LEFT JOIN usuarios U1 ON A.asignado_a1=U1.id_usuario 
  LEFT JOIN usuarios U2 ON A.asignado_a2=U2.id_usuario";
	$sql.=" WHERE A.estado = 'A' AND A.idruteoclas='".$id[0]; 
	$sql.="' ORDER BY A.fecha_create";
	$sql.=' LIMIT '.$pag.','.$regxPag;
	// echo $sql;
	$datos=datos_mysql($sql);
	return create_table($total,$datos["responseResult"],"seguiremoto",$regxPag,'../ruteo1/seguiRemoto.php');
}
...
function cmp_seguiremoto(){
 ...
 if ($e=="") {$e=$t;}
  ...
  $c[] = new cmp('acepta_abordaje','s','3',$d,$w.' FaP '.$o,'La familia acepta el abordaje presencial por gestor de Bienestar','rta',null,null,false,$u,'','col-25',"enbRutRmtAbo();");
  $c[] = new cmp('deriva_perfil1','s','50',$d,$w.' FaP FaY '.$o,'Perfil al que se deriva','perfil',null,null,false,$u,'','col-2',"selectDepend('deriva_perfil1','asignado_a1','seguiRemoto.php');");
  $c[] = new cmp('asignado_a1','s','50',$d,$w.' FaP FaY '.$o,'Asignado A:','nombre',null,null,false,$u,'','col-3');
  ...
  $c[] = new cmp('acepta_plan','s','3',$d,$w.' ApB '.$o,'La familia acepta el Plan de Bienestar','rta',null,null,false,$u,'','col-25',"enbRutRmtPlan();");
  $c[] = new cmp('deriva_perfil2','s','50',$d,$w.' ApB ApS '.$o,'Perfil al que se deriva','perfil',null,null,false,$u,'','col-2',"selectDepend('deriva_perfil2','asignado_a2','seguiRemoto.php');");
  $c[] = new cmp('asignado_a2','s','50',$d,$w.' ApB ApS '.$o,'Asignado A:','nombre',null,null,false,$u,'','col-3');
...
/* function get_seguiremoto(){
  if($_POST['id']=='0'){
    ...
    $id=divide($_POST['id']);
     $sql="SELECT CONCAT_WS('_',rm.id_ruteoremoto,rm.idruteoclas),r.priorizacion,r.tipo_prior,rm.fecha_seg,rm.numsegui,rm.estado_s,rm.motivo_estado,rm.gestante,rm.menor5,rm.cronico,rm.general,rm.nov_pri_fam1,rm.gestante_cpn,rm.nov_pri_fam2,rm.menor5_rpms,rm.menor5_riesgo,rm.nov_pri_fam3,rm.inasistente_12m,rm.inasistente_6_12m,rm.persona_mayor,rm.persona_discapacidad,rm.salud_mental,rm.nuevo_diagnostico,rm.ningun_diagnostico,rm.vacunacion_incompleta,rm.acepta_vacunacion,rm.barrera_salud,rm.agendamiento,rm.activacion_ruta,rm.sin_afiliacion,rm.acepta_afiliacion,rm.sujeto_abordaje,rm.acepta_abordaje,rm.deriva_perfil1,rm.asignado_a1,rm.sujeto_concertacion,rm.acepta_plan,rm.deriva_perfil2,rm.asignado_a2,rm.accion1,rm.desc_accion1,rm.accion2,rm.desc_accion2,rm.accion3,rm.desc_accion3,rm.observaciones,rm.continua_seguimiento,rm.fecha_prox_seguimiento,rm.motivo_no_continuidad
       FROM ruteo_remoto rm
        LEFT JOIN eac_ruteo r ON rm.idruteoclas=r.id_ruteo  
...
if($_POST['gestante']=='2' && $_POST['menor5']=='2' && $_POST['cronico']=='2' && $_POST['general']=='2'){
  return "msj['Error: Debe seleccionar al menos una condicion.']";
     ...
     $params = [
        ...
        param_null($_POST['numsegui']??'','i'),
        param_null($_POST['estado_s']??'','s'),
        param_null($_POST['motivo_estado']??'','s'),
        param_null($_POST['gestante']??'','s'),
        param_null($_POST['menor5']??'','s'),
        param_null($_POST['cronico']??'','s'),
        param_null($_POST['general']??'','s'),
        param_null($_POST['nov_pri_fam1']??'','s'),
        param_null($_POST['gestante_cpn']??'','s'),
        param_null($_POST['nov_pri_fam2']??'','s'),
        param_null($_POST['menor5_rpms']??'','s'),
        param_null($_POST['menor5_riesgo']??'','s'),
        param_null($_POST['nov_pri_fam3']??'','s'),
        param_null($_POST['inasistente_12m']??'','s'),
        param_null($_POST['inasistente_6_12m']??'','s'),
        param_null($_POST['persona_mayor']??'','s'),
        param_null($_POST['persona_discapacidad']??'','s'),
        param_null($_POST['salud_mental']??'','s'),
        param_null($_POST['nuevo_diagnostico']??'','s'),
        param_null($_POST['ningun_diagnostico']??'','s'),
        param_null($_POST['vacunacion_incompleta']??'','s'),
        param_null($_POST['acepta_vacunacion']??'','s'),
        param_null($_POST['barrera_salud']??'','s'),
        param_null($_POST['agendamiento']??'','s'),
        param_null($_POST['activacion_ruta']??'','s'),
        param_null($_POST['sin_afiliacion']??'','s'),
        param_null($_POST['acepta_afiliacion']??'','s'),
        param_null($_POST['sujeto_abordaje']??'','s'),
        param_null($_POST['acepta_abordaje']??'','s'),
        param_null($_POST['deriva_perfil1']??'','s'),
        param_null($_POST['asignado_a1']??'','s'),
        param_null($_POST['sujeto_concertacion']??'','s'),
        param_null($_POST['acepta_plan']??'','s'),
        param_null($_POST['deriva_perfil2']??'','s'),
        param_null($_POST['asignado_a2']??'','s'),
        param_null($_POST['accion1']??'','s'),
        param_null($_POST['desc_accion1']??'','s'),
        param_null($_POST['accion2']??'','s'),
        param_null($_POST['desc_accion2']??'','s'),
        param_null($_POST['accion3']??'','s'),
        param_null($_POST['desc_accion3']??'','s'),
        param_null($_POST['observaciones']??'','s'),
        param_null($_POST['continua_seguimiento']??'','s'),
        param_null($_POST['fecha_prox_seguimiento']??'','s'),
        param_null($_POST['motivo_no_continuidad']??'','s'),
