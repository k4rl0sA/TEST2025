...
function cmp_rutclasif(){
 $rta="";
 $t = ['id_rutclas'=>'','idrutges'=>'','preclasif'=>'','clasifica'=>'','tiposeg'=>'','riesgo'=>'','accion'=>'','fecha'=>'','accion1'=>'','desc_accion1'=>'','accion2'=>'','desc_accion2'=>'','accion3'=>'','desc_accion3'=>'','profesional'=>'','solic_agend'=>'','ruta'=>'','sectorial'=>'','intsectorial'=>'','entornos'=>'','aseguram'=>'','fecha_agenda'=>'','accion_1'=>'','fecha_agenda1'=>'','profesional1'=>'','accion_2'=>'','docu_confirm'=>'','tipo'=>''];
 $w='rutclasif';
 $d=get_rutclasif();
 if ($d=="") {$d=$t;}
 ...
 $c[]=new cmp('docu_confirm','nu','999999999999999999',$d['docu_confirm'],$w.' AGe '.$o,'Documento Confirmado  del Usuario','docu_confirm',null,null,true,$u,'','col-25');
 ...
 $c[]=new cmp('aseguramiento','s','2',$d['aseguram'],$w.' '.$cl,'Aseguramiento','rta',null,null,true,false,'','col-2');
...
function get_rutclasif(){
	if($_POST['id']=='0'){
		...
		$id=divide($_POST['id']);
		// var_dump($id);
		$sql="SELECT id_rutclas,idrutges,preclasif,clasifica,tiposeg,riesgo,accion1,desc_accion1,accion2,desc_accion2,accion3,desc_accion3,fecha,profesional,solic_agend,ruta,sectorial,intsectorial,entornos,aseguram,fecha_agenda,accion_1,fecha_agenda1,profesional1,accion_2,docu_confirm,tipo
...
function opc_perfil1nombre1($id=''){
  if($_REQUEST['id']!=''){	
    $sql = "SELECT id_usuario id,CONCAT(id_usuario,'-',nombre) usuario FROM usuarios right join apro_terr at ON id_usuario=at.doc_asignado  WHERE 
    perfil=(select descripcion from catadeta c where idcatalogo=218 and idcatadeta='{$_REQUEST['id']}' and estado='A') 
    and subred=(SELECT subred FROM usuarios WHERE id_usuario ='{$_SESSION['us_sds']}')   ORDER BY nombre";
