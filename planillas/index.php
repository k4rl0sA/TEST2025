...
async function cargarResumenFamiliar() {
	...
	if (doc != '' || tip != ''|| fec != '' || col != '') {	
  		...
  		data.forEach(row => {
  		  ...
  		  html += `<tr><td>${row.nombre} ${row.fecha_ultima}</td><td>${icono} </td><td>${row.fecha_ultima}</td></tr>`;
  		});
  		html += "</table>";
...
async function cargarResumenIndivi() {
    ...
    if (doc && tip && fec && col) {
        ...
        data.forEach(row => {
            ...
            html += `<tr><td>${row.idpeople}</td><td>${icono_alerta} ${row.fecha_ultima}</td><td>${icono_signos} ${row.fecha_ultima}</td></tr>`;
        });
        html += "</table>";
