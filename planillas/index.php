...
async function cargarResumenFamiliar() {
	...
	if (doc != '' || tip != ''|| fec != '' || col != '') {	
		  ...
		  let ajustar = (row.fecha_ultima && row.estado === 'Validar') ? `<i class='fa-solid fa-screwdriver-wrench icon' style='cursor:pointer;color:mediumblue;text-shadow: 3px 3px 1px rgb(255 210 0);font-size:18px;' title='Ajustar' id='${row.id || ''}' onclick=\"mostrar('person','pro',event,'','lib.php',7,'person');\"></i>` : '';// && 
