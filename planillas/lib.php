...
function family_planillas(){
    ...
    $items = [
        ...
        ( SELECT MAX(C2.fecha) FROM hog_plancuid C2 WHERE C2.idviv = $idfam AND C2.usu_creo = $id[3] AND C2.fecha >= (CASE WHEN DAY(CURDATE()) <= 5 THEN DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 MONTH), '%Y-%m-01') ELSE DATE_FORMAT(CURDATE(), '%Y-%m-01') END) AND C2.fecha < (CASE WHEN DAY(CURDATE()) <= 5 THEN DATE_FORMAT(CURDATE(), '%Y-%m-01') ELSE DATE_FORMAT(DATE_ADD(CURDATE(), INTERVAL 1 MONTH), '%Y-%m-01') END)) AS fecha_ultima;",
...
/*     $sql = "SELECT P.idpeople, CASE WHEN MAX(A.id_alert) IS NOT NULL OR MAX(Aw.fecha) IS NOT NULL THEN 'Completado' ELSE 'Validar' END AS estado_alerta,
    ...
    $sql = "SELECT P.idpeople, CASE WHEN MAX(A.id_alert) IS NOT NULL THEN 'Completado' ELSE 'Validar' END AS estado_alerta, COALESCE(MAX(A.fecha), MAX(Aw.fecha)) AS fecha_alerta_ultima, CASE WHEN MAX(S.id_signos) IS NOT NULL THEN 'Completado' ELSE 'Validar' END AS estado_signos, COALESCE(MAX(S.fecha_toma), MAX(Sw.fecha_toma)) AS fecha_signos_ultima FROM person P LEFT JOIN hog_alert A ON P.idpeople = A.idpeople AND A.fecha = '$id[2]' AND A.usu_creo = $id[3] LEFT JOIN hog_signos S ON P.idpeople = S.idpeople AND S.fecha_toma = '$id[2]' AND S.usu_create = $id[3] LEFT JOIN hog_alert Aw ON P.idpeople = Aw.idpeople AND Aw.usu_creo = $id[3] AND Aw.fecha BETWEEN (CASE WHEN DAY(CURDATE()) <= 5 THEN DATE_SUB(DATE_SUB(CURDATE(), INTERVAL DAY(CURDATE()) - 1 DAY), INTERVAL 1 MONTH) ELSE DATE_SUB(CURDATE(), INTERVAL DAY(CURDATE()) - 1 DAY) END) AND (CASE WHEN DAY(CURDATE()) <= 5 THEN LAST_DAY(DATE_SUB(CURDATE(), INTERVAL 1 MONTH)) ELSE LAST_DAY(CURDATE()) END) LEFT JOIN hog_signos Sw ON P.idpeople = Sw.idpeople AND Sw.usu_create = $id[3] AND Sw.fecha_toma BETWEEN (CASE WHEN DAY(CURDATE()) <= 5 THEN DATE_SUB(DATE_SUB(CURDATE(), INTERVAL DAY(CURDATE()) - 1 DAY), INTERVAL 1 MONTH) ELSE DATE_SUB(CURDATE(), INTERVAL DAY(CURDATE()) - 1 DAY) END) AND (CASE WHEN DAY(CURDATE()) <= 5 THEN LAST_DAY(DATE_SUB(CURDATE(), INTERVAL 1 MONTH)) ELSE LAST_DAY(CURDATE()) END) WHERE P.vivipersona = $idfam GROUP BY P.idpeople;";
