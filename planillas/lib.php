...
function cmp_planillas(){
    ...
    $c[]= new cmp('seguimiento','s',3,$d['seguimiento'] ,$w.' Pcf '.$o, 'Seguimiento','numsegui','','',true,false,'','col-2','',[],false,'lib.php',1);
    $c[]= new cmp('perfil','s',3,'',$w.' '.$o,'Perfil','perfil',null,'',true,true,'','col-15',"changeSelect('perfil','colaborador');");//  ,"enabDepeValu('perfil','uSR');
    $c[]= new cmp('colaborador','s',20,$d['colaborador'] ,$w.' uSR '.$o, 'Colaborador','colaborador','','',false,true,'','col-35',"cargarResumenPcf();cargarResumenFamiliar();cargarResumenIndivi();");//  ,"enabDepeValu('perfil','uSR');
...
function family_planillas(){
    ...
    $items = [
        ...
        'Tamizaje Apgar' => "SELECT CASE WHEN EXISTS (SELECT 1 FROM hog_carac C WHERE C.idfam = $idfam AND DATE(C.fecha) = DATE('$id[2]') AND C.usu_create = $id[3] AND C.estado = 'A' AND EXISTS (SELECT 1 FROM hog_tam_apgar A INNER JOIN person P ON A.idpeople = P.idpeople WHERE P.vivipersona = C.idfam AND DATE(A.fecha_toma) = DATE(C.fecha) AND A.usu_creo = C.usu_create)) THEN 'Completado' ELSE 'Validar' END AS Estado,(SELECT MAX(DATE(A.fecha_toma)) FROM hog_tam_apgar A INNER JOIN person P ON A.idpeople = P.idpeople WHERE P.vivipersona = $idfam AND A.usu_creo = $id[3] AND DATE(A.fecha_toma) >= (CASE WHEN DAY(CURDATE()) <= 5 THEN DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 MONTH), '%Y-%m-01') ELSE DATE_FORMAT(CURDATE(), '%Y-%m-01') END) AND DATE(A.fecha_toma) < (CASE WHEN DAY(CURDATE()) <= 5 THEN DATE_FORMAT(CURDATE(), '%Y-%m-01') ELSE DATE_FORMAT(DATE_ADD(CURDATE(), INTERVAL 1 MONTH), '%Y-%m-01') END)) AS fecha_ultima;"];
