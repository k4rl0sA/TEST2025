...
function toggleActionMenu(e, id) {
    ...
    /**
    * Habilita menú contextual móvil en cualquier tabla.
    * @param {string} tableSelector - Selector de la tabla (ej: '#roles-table')
    * @param {string} btnSelector - Selector del botón de acciones (ej: '.action-menu-btn')
    */
    function enableMobileRowActions(tableSelector, btnSelector) {
    ...
    // --- Orden y paginador ---
   /**
     * Actualiza los iconos de orden en la cabecera de una tabla.
     * @param {string} tableSelector - Selector de la tabla (ej: '#roles-table')
     * @param {string} sortField - Campo actual de orden
     * @param {string} sortDir - Dirección ('asc' o 'desc')
     */
    function updateSortIcons(tableSelector, sortField, sortDir) {
     ...
     /**
     * Renderiza un paginador universal.
     * @param {string} containerSelector - Selector del contenedor (ej: '#paginator')
     * @param {number} page - Página actual
     * @param {number} totalPages - Total de páginas
     * @param {function} onChange - Callback al cambiar de página (recibe el número de página)
     * @param {number} maxButtons - Máximo de botones visibles (opcional, default 5)
     */
    function renderPaginator(containerSelector, page, totalPages, onChange, maxButtons = 5) {
        const pag = document.querySelector(containerSelector);
        if (!pag) return;
        pag.innerHTML = '';
...
// --- Filtros tipo chips ---
 function createChip(label, value, key, onRemove) {
        const chip = document.createElement('span');
        chip.className = 'chip';
        chip.innerHTML = `<strong>${label}:</strong> ${value} <button class="chip-remove" title="Quitar filtro">&times;</button>`;
        if (typeof onRemove === 'function') {
            chip.querySelector('.chip-remove').onclick = () => onRemove(key);
        }
        return chip;
    }
    // Actualiza los chips de filtros activos
    function updateActiveFiltersChips(filters, optionsMap, chipsContainerId, countSpanId, onRemove) {
        const chipsList = document.getElementById(chipsContainerId);
        if (!chipsList) return;
        chipsList.innerHTML = '';
        let count = 0;
        for (const key in filters) {
            if (filters[key]) {
                ...
                if (optionsMap[key] && Array.isArray(optionsMap[key])) {
                    const found = optionsMap[key].find(opt => opt.value == filters[key]);
                    if (found) label = found.label;
                }
                chipsList.appendChild(createChip(key.charAt(0).toUpperCase() + key.slice(1), label, key, onRemove));
                count++;
            }
        }
    ...
    // Limpia los filtros y actualiza el formulario
    function clearFilters(filters, filterFormId, onClear) {
        document.getElementById(filterFormId).reset();
    ...
    // Cierra los menús contextuales al hacer clic fuera de ellos o al hacer scroll
    document.addEventListener('click', () => {
        document.querySelectorAll('.action-menu.show').forEach(menu => menu.classList.remove('show'));
        document.querySelectorAll('columns-panel.show').forEach(panel => panel.classList.remove('show'));
    });
   ...
   if (toggleFiltersBtn && filtersPanel) {
    ...
    //Modal o panel de ayuda
    function showHelpModal(contentHtml) {
...
// Cierra el modal al presionar Escape
    ...
    //obtener los items seleccionados en una tabla 
    /**
     * Obtiene los valores de los checkboxes seleccionados en una tabla.
     * @param {string} tableSelector - Selector de la tabla (ej: '#roles-table')
     * @param {string} itemCheckboxSelector - Selector de los checkboxes individuales (ej: '.select-role', '.select-menu', etc.)
     * @returns {Array} Array de valores seleccionados
     */
    function getSelectedTokens(tableSelector, itemCheckboxSelector) {
        return Array.from(document.querySelectorAll(`${tableSelector} ${itemCheckboxSelector}:checked`)).map(cb => cb.value);
    }

    //Mostrar información de rango en paginadores
    function showRangeInfo(page, pageSize, totalRows, rangeInfoId = 'range-info') {
       const start = totalRows === 0 ? 0 : ((page - 1) * pageSize) + 1;
       let end = page * pageSize;
       if (end > totalRows) end = totalRows;
       const el = document.getElementById(rangeInfoId);
       if (el) {
           el.textContent = `Mostrando ${start}-${end} de ${totalRows}`;
       }
    }   

    /**
 * Ejecuta una acción masiva en el backend para los ítems seleccionados.
 * @param {string} endpoint - URL del backend (ej: '/adm-roles/lib.php?a=bulk')
 * @param {string} action - Acción a realizar ('activate', 'inactivate', 'delete', etc.)
 * @param {Array} tokens - Array de tokens seleccionados
 * @param {function} onSuccess - Callback en éxito
 * @param {function} onError - Callback en error
 */
function bulkAction(endpoint, action, tokens, onSuccess = null, onError = null) {
    const formData = new FormData();
    formData.append('csrf_token', window.CSRF_TOKEN);
    formData.append('action', action);
    formData.append('tokens', JSON.stringify(tokens));
    fetchWithLoader(endpoint, {
        method: 'POST',
        body: formData
    }, onSuccess, onError);
}

function updateBulkActionsBar(tableSelector, itemCheckboxSelector, barId , countId ) {
    const selected = getSelectedTokens(tableSelector, itemCheckboxSelector);
    console.log('updateBulkActionsBar', selected.length, selected); // <-- Depuración
    const bar = document.getElementById(barId);
    const count = document.getElementById(countId);
    if (!bar || !count) return;
    if (selected.length > 0) {
        bar.classList.remove('hidden');
        count.textContent = `${selected.length} items`;
    } else {
...
/**
 * Inicializa la selección masiva en cualquier tabla.
 * @param {string} tableSelector - Selector de la tabla (ej: '#roles-table')
 * @param {string} selectAllId - ID del checkbox del encabezado (ej: 'select-all-roles')
 * @param {string} itemCheckboxSelector - Selector de los checkboxes individuales (ej: '.select-role', '.select-menu', etc.)
 * @param {function} updateFn - Callback para actualizar barra de acciones masivas
 */
function initBulkSelection(tableSelector, selectAllId, itemCheckboxSelector, updateFn) {
    const selectAll = document.getElementById(selectAllId);
    if (selectAll) {
        selectAll.onchange = function() {
            const checked = this.checked;
            document.querySelectorAll(`${tableSelector} ${itemCheckboxSelector}`).forEach(cb => {
                cb.checked = checked;
            });
            updateFn();
        };
    ...
    document.querySelectorAll(`${tableSelector} ${itemCheckboxSelector}`).forEach(cb => {
        cb.onchange = function() {
            updateFn();
            // Sincroniza el checkbox del encabezado
            if (selectAll) {
                const all = document.querySelectorAll(`${tableSelector} ${itemCheckboxSelector}`);
                const checkedCount = document.querySelectorAll(`${tableSelector} ${itemCheckboxSelector}:checked`).length;
...
document.addEventListener('click', function(e) {
    ...
    // Si el clic NO fue dentro del panel ni en el botón, oculta el panel
...
document.addEventListener('click', function(e) {
    document.querySelectorAll('div[id^="columns-panel"]').forEach(panel => {
        const btnId = panel.id.replace('columns-panel', 'toggle-columns-btn');
...
function saveColumnPreferences(moduleKey) {
    const prefs = {};
    document.querySelectorAll(`#columns-panel-${moduleKey} .col-toggle`).forEach(cb => {
        prefs[cb.dataset.col] = cb.checked;
    });
    localStorage.setItem(`${moduleKey}_column_prefs`, JSON.stringify(prefs));
}

function loadColumnPreferences(moduleKey) {
    const prefs = JSON.parse(localStorage.getItem(`${moduleKey}_column_prefs`) || '{}');
    document.querySelectorAll(`#columns-panel-${moduleKey} .col-toggle`).forEach(cb => {
...
document.addEventListener('DOMContentLoaded', function() {
    // Busca todos los botones de personalización de columnas
    document.querySelectorAll('button[id^="toggle-columns-btn"]').forEach(btn => {
        const panelId = btn.id.replace('toggle-columns-btn', 'columns-panel');
