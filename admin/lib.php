...
function lis_soporte($txt){
	$sql="SELECT S.idsoporte AS Ticket,S.idpeople AS Cod_Persona,S.documento AS Documento,FN_CATALOGODESC(1, S.tipo_doc) AS Tipo_Documento,FN_CATALOGODESC(21, S.sexo) AS Sexo,S.fecha_nacio AS Fecha_Nacimiento,S.cod_predio AS Cod_Predio,S.cod_familia AS Cod_Familia,S.cod_registro AS Cod_Registro_Cantidad,FN_CATALOGODESC(286, S.formulario) AS Solicitud,S.error AS Error,FN_CATALOGODESC(72, S.ok) AS OK-Subred_Aprueba,FN_CATALOGODESC(201, S.prioridad) AS Prioridad,S.observaciones AS Observaciones,S.aprueba AS Perfil_Aprueba,U.nombre AS Usuario_Creo,U.subred AS Subred,S.fecha_create AS Fecha_Creacion,CASE     WHEN S.usu_update IS NULL THEN 'PENDIENTE'     ELSE COALESCE(U1.nombre, 'USUARIO NO ENCONTRADO')END AS Usuario_Resuelto,CASE     WHEN S.fecha_update IS NULL THEN 'PENDIENTE'     ELSE DATE_FORMAT(S.fecha_update, '%Y-%m-%d %H:%i:%s')END AS Fecha_Respuesta,CASE     WHEN S.estado IS NULL THEN 'PENDIENTE'    WHEN S.estado = 1 THEN 'REALIZADO'    ELSE FN_CATALOGODESC(285, S.estado)END AS Estado
FROM `soporte` S
