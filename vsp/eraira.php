...
function gra_eraira(){
  ...
  $campos = [
    ...
    'asiste_control', 'vacuna_comple', 'lacmate_exclu', 'lacmate_comple', 'alime_complemen', 'adecua_oxi', 'adhe_tratam',
    'signos_alarma', 'signos_alarma_seg', 'adhe_lavamano', 'reing_hospita',
  ...
  if(count($id)==4){
    ...
    $sql = "UPDATE vsp_eraira SET "
      ...
      . "WHERE id_eraira = ?";
    $params[] = ['type' => 's', 'value' => $id[0]]; // id_eraira
    ...
    $sql = "INSERT INTO vsp_eraira (
      id_eraira, " . implode(', ', $campos) . "
    ) VALUES (
      NULL, $placeholders
    )";
    $rta = mysql_prepd($sql, $params);
  } else {
    $rta = "Error: id_eraira inválido";
  }
  return $rta;
} 
