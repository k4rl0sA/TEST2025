...
function focus_feminicidio(){
  return 'feminicidio';
 }
 
 
 function men_feminicidio(){
  $rta=cap_menus('feminicidio','pro');
  return $rta;
 }
 ...
 function cap_menus($a,$b='cap',$con='con') {
   $rta = ""; 
   $acc=rol($a);
   	if ($a=='feminicidio'  && isset($acc['crear']) && $acc['crear']=='SI'){  
	 $rta .= "<li class='icono feminicidio grabar'      title='Grabar'          OnClick=\"grabar('$a',this);\"></li>";
    }
  return $rta;
 }


 FUNCTION lis_feminicidio(){
	// var_dump($_POST['id']);
	$id = isset($_POST['id']) ? divide($_POST['id']) : (isset($_POST['id_feminicidio']) ? divide($_POST['id_feminicidio']) : null);
  $info=datos_mysql("SELECT COUNT(*) total FROM vsp_femini A LEFT JOIN  usuarios U ON A.usu_creo=U.id_usuario 
  WHERE A.estado = 'A' AND A.idpeople='".$id[0]."'");
	$total=$info['responseResult'][0]['total'];
	$regxPag=4;
  $pag=(isset($_POST['pag-feminicidio']))? ($_POST['pag-feminicidio']-1)* $regxPag:0;

	$sql="SELECT `id_femini` ACCIONES,id_femini 'Cod Registro',
...
FROM vsp_femini A
  ...
  LEFT JOIN   person P ON A.idpeople=P.idpeople";
	...
	return create_table($total,$datos["responseResult"],"feminicidio",$regxPag,'../vsp/feminicidio.php');
   }


function cmp_feminicidio(){
	$rta="<div class='encabezado'>TABLA SEGUIMIENTOS FEMINICIDIO</div>
	<div class='contenido' id='feminicidio-lis'>".lis_feminicidio()."</div></div>";
	$w='feminicidio';
  $d='';
	$o='inf';
  $ob='Ob';
  ...
  $ev=$event[2];
  $days=fechas_app('vsp');
  $c[]=new cmp('id_femini','h','50',$_POST['id'],$w.' '.$o,'Id de Feminicidio','id_femini',null,null,false,false,'','col-2');
  ...
  $c[]=new cmp('motivo_estado','s','3',$d,$w.' '.$o,'Motivo de Estado','motivo_estado',null,null,false,$x,'','col-2');
    ...
    $o='hab';
    $c[]=new cmp($o,'e',null,'EVALUACIÓN CONDICIONES DE VIDA ',$w);
    $c[]=new cmp('descanso_adecuado','s','2',$d,$w.' hL '.$o,'¿Ha podido descansar adecuadamente?','rta',null,null,false,$x,'','col-3');
    $c[]=new cmp('molestia_fisica','s','2',$d,$w.' hL '.$o,'¿Ha tenido alguna molestia física en los últimos días?','rta',null,null,false,$x,'','col-3');
    $c[]=new cmp('malestar_emocional','s','2',$d,$w.' hL '.$o,'¿Refiere algún malestar emocional general?','rta',null,null,false,$x,'','col-3');
    $c[]=new cmp('convive_agresor','s','2',$d,$w.' hL '.$o,'¿Actualmente convive con la persona asociada al VBG?','convive_agresor',null,null,false,$x,'','col-3');
    $c[]=new cmp('cambios_entorno','s','2',$d,$w.' hL '.$o,'¿Ha sentido cambios en su entorno desde la última visita?','cambios_entorno',null,null,false,$x,'','col-3');
    $c[]=new cmp('preocupacion_hogar','s','2',$d,$w.' hL '.$o,'¿En algún momento reciente sintió preocupación por una situación del hogar?','rta',null,null,false,$x,'','col-3');
    ...
    $c[]=new cmp('acciones_1','s','3',$d,$w.' '.$o,'Accion 1','acciones_1',null,null,false,$x,'','col-5','selectDepend(\'acciones_1\',\'desc_accion1\',\'../vsp/feminicidio.php\');');
    $c[]=new cmp('desc_accion1','s','3',$d,$w.' '.$o,'Descripcion Accion 1','desc_accion1',null,null,false,$x,'','col-5');
    $c[]=new cmp('acciones_2','s','3',$d,$w.' '.$no.' '.$o,'Accion 2','acciones_2',null,null,false,$x,'','col-5','selectDepend(\'acciones_2\',\'desc_accion2\',\'../vsp/feminicidio.php\');');
    $c[]=new cmp('desc_accion2','s','3',$d,$w.' '.$no.' '.$o,'Descripcion Accion 2','desc_accion2',null,null,false,$x,'','col-5');
    $c[]=new cmp('acciones_3','s','3',$d,$w.' '.$no.' '.$o,'Accion 3','acciones_3',null,null,false,$x,'','col-5','selectDepend(\'acciones_3\',\'desc_accion3\',\'../vsp/feminicidio.php\');');
...
function opc_equ(){
  ...
  function gra_feminicidio() {
    $id = divide($_POST['id_femini']);
    $eq = opc_equ();
    $smbin = null;
    ...
    $campos = [
        ...
        'descanso_adecuado', 'molestia_fisica', 'malestar_emocional', 'convive_agresor', 'cambios_entorno', 'preocupacion_hogar',
    ...
    if (count($id) == 4) { // UPDATE
        ...
        $sql = "UPDATE vsp_femini SET "
            . implode(' = ?, ', $set) . " = ?, usu_update = ?, fecha_update = NOW() "
            . "WHERE id_femini = ?";
        $params[] = ['type' => 's', 'value' => $id[0]]; // id_femini
        ...
        $sql = "INSERT INTO vsp_femini (
            id_femini, " . implode(', ', $campos) . "
        ) VALUES (
            NULL, $placeholders
        )";
        $rta = mysql_prepd($sql, $params);
    } else {
        $rta = "Error: id_femini inválido";
    }
    return $rta;
}

  function get_feminicidio(){
    if($_REQUEST['id']==''){
      return "";
    }else{
      $id=divide($_REQUEST['id']);
      $sql="SELECT concat_ws('_',id_femini,idpeople,numsegui,evento),
      fecha_seg,numsegui,evento,estado_s,motivo_estado,descanso_adecuado,molestia_fisica,malestar_emocional,convive_agresor,cambios_entorno,preocupacion_hogar,estrategia_1,estrategia_2,acciones_1,desc_accion1,acciones_2,desc_accion2,acciones_3,desc_accion3,activa_ruta,ruta,novedades,signos_covid,caso_afirmativo,otras_condiciones,observaciones,cierre_caso,motivo_cierre,fecha_cierre,liker_dificul,liker_emocion,liker_decision,redu_riesgo_cierre,users_bina
      FROM vsp_femini
      WHERE id_femini ='{$id[0]}'";
      // echo $sql;
      // print_r($id);
      $info=datos_mysql($sql);
...
// var_dump($a);
// var_dump($rta);
	if ($a=='feminicidio' && $b=='acciones'){
		$rta="<nav class=getData'menu right'>";	
		$rta.="<li class='icono editar' title='Editar' id='".$c['ACCIONES']."' Onclick=\"setTimeout(getData,500,'feminicidio',event,this,['fecha_seg','numsegui','evento','estado_s','motivo_estado','cierre_caso'],'../vsp/feminicidio.php');\"></li>";
	}
