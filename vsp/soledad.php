<?php
require_once "../libs/gestion.php";
ini_set('display_errors','1');
if ($_POST['a']!='opc') $perf=perfil($_POST['tb']);
if (!isset($_SESSION['us_sds'])) die("<script>window.top.location.href='/';</script>");
else {
  $rta="";
  switch ($_POST['a']){
  case 'csv': 
    header_csv ($_REQUEST['tb'].'.csv');
    $rs=array('','');    
    echo csv($rs,'');
    die;
    break;
  default:
    eval('$rta='.$_POST['a'].'_'.$_POST['tb'].'();');
    if (is_array($rta)) json_encode($rta);
	else echo $rta;
  }   
}

function focus_soledad(){
  return 'soledad';
}

function men_soledad(){
  $rta=cap_menus('soledad','pro');
  return $rta;
}

function cap_menus($a,$b='cap',$con='con') {
  $rta = "";
  $acc=rol($a);
  if ($a=='soledad' && isset($acc['crear']) && $acc['crear']=='SI'){  
    $rta .= "<li class='icono $a grabar' title='Grabar' OnClick=\"grabar('$a',this);\"></li>";
  }
  return $rta;
}

function lis_soledad(){
  $id = isset($_POST['id']) ? divide($_POST['id']) : (isset($_POST['id_soledad']) ? divide($_POST['id_soledad']) : null);
  $info=datos_mysql("SELECT COUNT(*) total FROM vsp_soledad A 
    LEFT JOIN usuarios U ON A.usu_creo=U.id_usuario 
    WHERE A.estado = 'A' AND A.idpeople='".$id[0]."'");
  $total=$info['responseResult'][0]['total'];
  $regxPag=4;
  $pag=(isset($_POST['pag-soledad']))? ($_POST['pag-soledad']-1)* $regxPag:0;

  $sql="SELECT `id_soledad` ACCIONES, id_soledad 'Cod Registro',
    P.tipo_doc, P.idpersona, fecha_seg Fecha, numsegui Seguimiento, 
    FN_CATALOGODESC(87,evento) EVENTO, FN_CATALOGODESC(73,estado_s) Estado, 
    cierre_caso Cierra, fecha_cierre 'Fecha de Cierre', nombre Creó 
    FROM vsp_soledad A
    LEFT JOIN usuarios U ON A.usu_creo=U.id_usuario 
    LEFT JOIN person P ON A.idpeople=P.idpeople";
  $sql.=" WHERE A.estado = 'A' AND A.idpeople='".$id[0]; 
  $sql.="' ORDER BY A.fecha_create";
  $sql.=' LIMIT '.$pag.','.$regxPag;
  $datos=datos_mysql($sql);
  return create_table($total,$datos["responseResult"],"soledad",$regxPag,'../vsp/soledad.php');
}

function cmp_soledad(){
  $rta="<div class='encabezado'>TABLA SEGUIMIENTOS - SOLEDAD</div>
  <div class='contenido' id='soledad-lis'>".lis_soledad()."</div></div>";
  $w='soledad';
  $d='';
  $o='inf';
  $ob='Ob';
  $no='nO';
  $bl='bL';
  $x=false;
  $block=['red','emo','fun'];
  $event=divide($_POST['id']);
  $ev=$event[2];
  $days=fechas_app('vsp');
  
  $c[]=new cmp('id_soledad','h','50',$_POST['id'],$w.' '.$o,'','id_soledad',null,null,false,$x,'','col-2');
  $c[]=new cmp('fecha_seg','d','10',$d,$w.' '.$o,'Fecha Seguimiento','fecha_seg',null,null,true,true,'','col-2',"validDate(this,$days,0);");
  $c[]=new cmp('numsegui','s','3',$d,$w.' '.$o,'Seguimiento N°','numsegui',null,null,true,true,'','col-2',"staEfe('numsegui','sta');EnabEfec(this,['red','emo','fun'],['Ob'],['nO'],['bL'])");
  $c[]=new cmp('evento','s','3',$ev,$w.' '.$o,'Evento','evento',null,null,false,false,'','col-2');
  $c[]=new cmp('estado_s','s','3',$d,$w.' sTa '.$o,'Estado','estado_s',null,null,true,true,'','col-2',"enabFielSele(this,true,['motivo_estado'],['3']);EnabEfec(this,['red','emo','fun'],['Ob'],['nO'],['bL']);");
  $c[]=new cmp('motivo_estado','s','3',$d,$w.' '.$o,'Motivo de Estado','motivo_estado',null,null,false,$x,'','col-2');
  
  // DIMENSIÓN 1: RED DE APOYO SOCIAL
  $o='red';
  $c[]=new cmp($o,'e',null,'DIMENSIÓN 1: RED DE APOYO SOCIAL',$w);
  $c[]=new cmp('frec_comunicacion','s','2',$d,$w.' '.$o,'1.1 ¿Con qué frecuencia conversa o se comunica con familiares, amigos o vecinos?','frec_comunicacion',null,null,false,$x,'','col-5');
  $c[]=new cmp('tiene_acompanante','s','2',$d,$w.' '.$o,'1.2 ¿Cuenta con alguien que lo/la visite o lo/la acompañe de manera regular?','rta',null,null,false,$x,'','col-5');
  $c[]=new cmp('tiene_contacto_emerg','s','2',$d,$w.' '.$o,'1.3 ¿Tiene a quién llamar en caso de emergencia o si se siente mal?','rta',null,null,false,$x,'','col-5');
  $c[]=new cmp('participa_comunidad','s','2',$d,$w.' '.$o,'1.4 ¿Participa en alguna actividad comunitaria, grupo o espacio social?','rta',null,null,false,$x,'','col-5');
  
  // DIMENSIÓN 2: PERCEPCIÓN EMOCIONAL
  $o='emo';
  $c[]=new cmp($o,'e',null,'DIMENSIÓN 2: PERCEPCIÓN EMOCIONAL',$w);
  $c[]=new cmp('frec_soledad','s','2',$d,$w.' '.$o,'2.1 ¿Con qué frecuencia se siente solo/a, aunque tenga contactos o llamadas?','frec_soledad',null,null,false,$x,'','col-5');
  $c[]=new cmp('percepcion_compania','s','2',$d,$w.' '.$o,'2.2 ¿Siente que tiene suficiente compañía o alguien con quien compartir?','percepcion_compania',null,null,false,$x,'','col-5');
  $c[]=new cmp('tristeza_reciente','s','2',$d,$w.' '.$o,'2.3 En las últimas dos semanas, ¿ha sentido tristeza, desánimo o falta de interés?','rta',null,null,false,$x,'','col-5');
  $c[]=new cmp('estado_animo','s','2',$d,$w.' '.$o,'2.4 ¿Cómo describiría su estado de ánimo la mayor parte del tiempo?','estado_animo',null,null,false,$x,'','col-5');
  
  // DIMENSIÓN 3: FUNCIONALIDAD Y RIESGO
  $o='fun';
  $c[]=new cmp($o,'e',null,'DIMENSIÓN 3: FUNCIONALIDAD Y RIESGO',$w);
  $c[]=new cmp('dificultades_adl','m','100',$d,$w.' '.$o,'3.1 ¿Tiene dificultades para realizar alguna de estas actividades sin ayuda? (Múltiples)','dificultades_adl',null,null,false,$x,'','col-5');
  $c[]=new cmp('riesgo_reciente','s','2',$d,$w.' '.$o,'3.2 ¿Ha tenido alguna caída, accidente o situación de riesgo reciente estando solo/a?','rta',null,null,false,$x,'','col-5');
  $c[]=new cmp('recibe_seguimiento','s','2',$d,$w.' '.$o,'3.3 ¿Recibe visitas o seguimiento de algún servicio de salud o programa de apoyo social?','rta',null,null,false,$x,'','col-5');
  
  // ACCIONES DEL PLAN FAMILIAR
  $o='acc';
  $c[]=new cmp($o,'e',null,'ACCIONES DEL PLAN FAMILIAR',$w);
  $c[]=new cmp('estrategia_1','s','3',$d,$w.' '.$o,'Estrategia PF_1','estrategia_1',null,null,false,$x,'','col-5');
  $c[]=new cmp('estrategia_2','s','3',$d,$w.' '.$no.' '.$o,'Estrategia PF_2','estrategia_2',null,null,false,$x,'','col-5');
  $c[]=new cmp('acciones_1','s','3',$d,$w.' '.$o,'Acción 1','acciones_1',null,null,false,$x,'','col-5',"selectDepend('acciones_1','desc_accion1','../vsp/soledad.php');");
  $c[]=new cmp('desc_accion1','s','3',$d,$w.' '.$o,'Descripción Acción 1','desc_accion1',null,null,false,$x,'','col-5');
  $c[]=new cmp('acciones_2','s','3',$d,$w.' '.$no.' '.$o,'Acción 2','acciones_2',null,null,false,$x,'','col-5',"selectDepend('acciones_2','desc_accion2','../vsp/soledad.php');");
  $c[]=new cmp('desc_accion2','s','3',$d,$w.' '.$no.' '.$o,'Descripción Acción 2','desc_accion2',null,null,false,$x,'','col-5');
  $c[]=new cmp('acciones_3','s','3',$d,$w.' '.$no.' '.$o,'Acción 3','acciones_3',null,null,false,$x,'','col-5',"selectDepend('acciones_3','desc_accion3','../vsp/soledad.php');");
  $c[]=new cmp('desc_accion3','s','3',$d,$w.' '.$no.' '.$o,'Descripción Acción 3','desc_accion3',null,null,false,$x,'','col-5');
  
  $c[]=new cmp('activa_ruta','s','2',$d,$w.' '.$o,'Ruta Activada','rta',null,null,false,$x,'','col-3',"enabRuta(this,'rt');");
  $c[]=new cmp('ruta','s','3',$d,$w.' '.$no.' rt '.$bl.' '.$o,'Ruta','ruta',null,null,false,$x,'','col-35');
  $c[]=new cmp('novedades','s','3',$d,$w.' '.$no.' '.$o,'Novedades','novedades',null,null,false,$x,'','col-35');
  $c[]=new cmp('signos_covid','s','2',$d,$w.' '.$o,'¿Signos y Síntomas para Covid19?','rta',null,null,false,$x,'','col-2',"enabCovid(this,'cv');");
  $c[]=new cmp('caso_afirmativo','t','500',$d,$w.' cv '.$bl.' '.$no.' '.$o,'Relacione Cuales signos y síntomas, Y Atención Recibida Hasta el Momento','caso_afirmativo',null,null,false,$x,'','col-4');
  $c[]=new cmp('otras_condiciones','t','500',$d,$w.' cv '.$bl.' '.$no.' '.$o,'Otras Condiciones de Riesgo que Requieren una Atención Complementaria.','otras_condiciones',null,null,false,$x,'','col-4');
  $c[]=new cmp('observaciones','a','1500',$d,$w.' '.$ob.' '.$o,'Observaciones','observaciones',null,null,true,true,'','col-10');
  $c[]=new cmp('cierre_caso','s','2',$d,$w.' '.$ob.' '.$o,'Cierre de Caso','rta',null,null,true,true,'','col-2','enabFincas(this,\'cc\');');
  $c[]=new cmp('fecha_cierre','d','10',$d,$w.' cc '.$bl.' '.$no.' '.$o,'Fecha de Cierre','fecha_cierre',null,null,false,$x,'','col-25',"validDate(this,$days,0);");
  $c[]=new cmp('motivo_cierre','s','3',$d,$w.' cc '.$bl.' '.$no.' '.$o,'Motivo Cierre','motivo_cierre',null,null,false,$x,'','col-55');
  $c[]=new cmp('redu_riesgo_cierre','s','2',$d,$w.' cc '.$bl.' '.$no.' '.$o,'¿Reducción del riesgo?','rta',null,null,false,$x,'','col-15');
  $c[]=new cmp('users_bina[]','m','60',$d,$w.' '.$ob.' '.$o,'Usuarios Equipo','bina',null,null,false,true,'','col-5');
  
  for ($i=0;$i<count($c);$i++) $rta.=$c[$i]->put();
  return $rta;
}

// FUNCIONES OPC PARA SELECT OPTIONS
function opc_bina($id=''){
  return opc_sql("SELECT id_usuario, nombre FROM usuarios u WHERE equipo=(select equipo from usuarios WHERE id_usuario='{$_SESSION['us_sds']}') and estado='A' ORDER BY 2;",$id);
}
function opc_motivo_cierre($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion,valor FROM `catadeta` WHERE idcatalogo=198 and estado='A' ORDER BY 1",$id);
}
function opc_rta($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=170 and estado='A' ORDER BY 1",$id);
}
function opc_numsegui($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=76 and estado='A' ORDER BY LENGTH(idcatadeta), idcatadeta",$id);
}
function opc_evento($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=87 and estado='A' ORDER BY 1",$id);
}
function opc_estado_s($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=73 and estado='A' ORDER BY 1",$id);
}
function opc_motivo_estado($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=74 and estado='A' ORDER BY 1",$id);
}
function opc_frec_comunicacion($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=331 and estado='A' ORDER BY 1",$id);
}
function opc_frec_soledad($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=135 and estado='A' ORDER BY 1",$id);
}
function opc_percepcion_compania($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=332 and estado='A' ORDER BY 1",$id);
}
function opc_estado_animo($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=333 and estado='A' ORDER BY 1",$id);
}
function opc_dificultades_adl($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=334 and estado='A' ORDER BY 1",$id);
}
function opc_estrategia_1($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=90 and estado='A' ORDER BY 1",$id);
}
function opc_estrategia_2($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=90 and estado='A' ORDER BY 1",$id);
}
function opc_acciones_1($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=22 and estado='A' ORDER BY 1",$id);
}
function opc_desc_accion1($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=75 and estado='A' ORDER BY 1",$id);
}
function opc_acciones_1desc_accion1($id=''){
  if($_REQUEST['id']!=''){
    $id=divide($_REQUEST['id']);
    $sql="SELECT idcatadeta, descripcion FROM `catadeta` WHERE idcatalogo=75 and estado='A' and valor='".$id[0]."' ORDER BY LENGTH(idcatadeta), idcatadeta;";
    $info=datos_mysql($sql);		
    return json_encode($info['responseResult']);
  }
}
function opc_acciones_2desc_accion2($id=''){
  if($_REQUEST['id']!=''){
    $id=divide($_REQUEST['id']);
    $sql="SELECT idcatadeta, descripcion FROM `catadeta` WHERE idcatalogo=75 and estado='A' and valor='".$id[0]."' ORDER BY LENGTH(idcatadeta), idcatadeta;";
    $info=datos_mysql($sql);		
    return json_encode($info['responseResult']);
  }
}
function opc_acciones_3desc_accion3($id=''){
  if($_REQUEST['id']!=''){
    $id=divide($_REQUEST['id']);
    $sql="SELECT idcatadeta 'id', descripcion 'asc' FROM `catadeta` WHERE idcatalogo=75 and estado='A' and valor='".$id[0]."' ORDER BY LENGTH(idcatadeta), idcatadeta;";
    $info=datos_mysql($sql);		
    return json_encode($info['responseResult']);
  }
}
function opc_acciones_2($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=22 and estado='A' ORDER BY 1",$id);
}
function opc_desc_accion2($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=75 and estado='A' ORDER BY 1",$id);
}
function opc_acciones_3($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=22 and estado='A' ORDER BY 1",$id);
}
function opc_desc_accion3($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=75 and estado='A' ORDER BY 1",$id);
}
function opc_ruta($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=79 and estado='A' ORDER BY 1",$id);
}
function opc_novedades($id=''){
  return opc_sql("SELECT `idcatadeta`,descripcion FROM `catadeta` WHERE idcatalogo=77 and estado='A' ORDER BY 1",$id);
}
function opc_equ(){
  $sql="SELECT equipo FROM usuarios WHERE id_usuario='{$_SESSION['us_sds']}'";
  $info=datos_mysql($sql);		
  return $info['responseResult'][0]['equipo'];
}

// FUNCIÓN GRABAR
function gra_soledad(){
  $id = divide($_POST['id_soledad']);
  $eq = opc_equ();
  $smbin = null;
  if (($smbina = $_POST['fusers_bina'] ?? null) && is_array($smbina)) {
    $smbin = implode(",", str_replace("'", "", $smbina));
  }

  // Manejo de campo múltiple dificultades_adl
  $dificultades = '';
  if (isset($_POST['fdificultades_adl']) && is_array($_POST['fdificultades_adl'])) {
    $dificultades = implode(",", str_replace("'", "", $_POST['fdificultades_adl']));
  }

  // Orden de los campos según la tabla
  $campos = [
    'idpeople', 'fecha_seg', 'numsegui', 'evento', 'estado_s', 'motivo_estado',
    'frec_comunicacion', 'tiene_acompanante', 'tiene_contacto_emerg', 'participa_comunidad',
    'frec_soledad', 'percepcion_compania', 'tristeza_reciente', 'estado_animo',
    'dificultades_adl', 'riesgo_reciente', 'recibe_seguimiento',
    'estrategia_1', 'estrategia_2', 'acciones_1', 'desc_accion1', 'acciones_2', 'desc_accion2', 
    'acciones_3', 'desc_accion3', 'activa_ruta', 'ruta', 'novedades', 
    'signos_covid', 'caso_afirmativo', 'otras_condiciones', 'observaciones',
    'cierre_caso', 'motivo_cierre', 'fecha_cierre', 'redu_riesgo_cierre', 
    'users_bina', 'equipo_bina', 'usu_creo', 'usu_update', 'fecha_update', 'estado'
  ];
  
  // Campos fecha que pueden ser nulos
  $campos_fecha_null = ['fecha_cierre', 'fecha_update'];

  if(count($id)==4){
    // UPDATE
    $set = [
      'frec_comunicacion', 'tiene_acompanante', 'tiene_contacto_emerg', 'participa_comunidad',
      'frec_soledad', 'percepcion_compania', 'tristeza_reciente', 'estado_animo',
      'dificultades_adl', 'riesgo_reciente', 'recibe_seguimiento',
      'estrategia_1', 'estrategia_2', 'acciones_1', 'desc_accion1', 'acciones_2', 'desc_accion2', 
      'acciones_3', 'desc_accion3', 'activa_ruta', 'ruta', 'novedades', 
      'signos_covid', 'caso_afirmativo', 'otras_condiciones', 'observaciones',
      'cierre_caso', 'motivo_cierre', 'fecha_cierre', 'redu_riesgo_cierre', 
      'users_bina', 'equipo_bina'
    ];
    $params = [];
    foreach ($set as $campo) {
      if ($campo == 'users_bina') {
        $params[] = ['type' => 's', 'value' => $smbin];
      } elseif ($campo == 'equipo_bina') {
        $params[] = ['type' => 's', 'value' => $eq];
      } elseif ($campo == 'dificultades_adl') {
        $params[] = ['type' => 's', 'value' => $dificultades];
      } elseif (in_array($campo, $campos_fecha_null)) {
        $val = $_POST[$campo] ?? null;
        $params[] = [
          'type' => ($val === '' || $val === null) ? 'z' : 's',
          'value' => ($val === '' || $val === null) ? null : $val
        ];
      } else {
        $params[] = ['type' => 's', 'value' => $_POST[$campo] ?? null];
      }
    }
    $params[] = ['type' => 's', 'value' => $_SESSION['us_sds']]; // usu_update
    $sql = "UPDATE vsp_soledad SET "
      . implode(' = ?, ', $set) . " = ?, usu_update = ?, fecha_update = DATE_SUB(NOW(), INTERVAL 5 HOUR) "
      . "WHERE id_soledad = ?";
    $params[] = ['type' => 's', 'value' => $id[0]]; // id_soledad
    $rta = mysql_prepd($sql, $params);

  } else if(count($id)==3){
    // INSERT
    $params = [];
    foreach ($campos as $campo) {
      if ($campo == 'idpeople') {
        $params[] = ['type' => 's', 'value' => $id[0]];
      } elseif ($campo == 'users_bina') {
        $params[] = ['type' => 's', 'value' => $smbin];
      } elseif ($campo == 'equipo_bina') {
        $params[] = ['type' => 's', 'value' => $eq];
      } elseif ($campo == 'usu_creo') {
        $params[] = ['type' => 's', 'value' => $_SESSION['us_sds']];
      } elseif ($campo == 'usu_update' || $campo == 'fecha_update' || $campo == 'fecha_create') {
        $params[] = ['type' => 'z', 'value' => null];
      } elseif ($campo == 'estado') {
        $params[] = ['type' => 's', 'value' => 'A'];
      } elseif ($campo == 'dificultades_adl') {
        $params[] = ['type' => 's', 'value' => $dificultades];
      } elseif (in_array($campo, $campos_fecha_null)) {
        $val = $_POST[$campo] ?? null;
        $params[] = [
          'type' => ($val === '' || $val === null) ? 'z' : 's',
          'value' => ($val === '' || $val === null) ? null : $val
        ];
      } else {
        $params[] = ['type' => 's', 'value' => $_POST[$campo] ?? null];
      }
    }
    $placeholders = implode(', ', array_fill(0, count($params), '?'));
    $sql = "INSERT INTO vsp_soledad (
      id_soledad, " . implode(', ', $campos) . "
    ) VALUES (
      NULL, $placeholders
    )";
    $rta = mysql_prepd($sql, $params);
  } else {
    $rta = "Error: id_soledad inválido";
  }
  return $rta;
}

// FUNCIÓN GET
function get_soledad(){
  if($_REQUEST['id']==''){
    return "";
  }else{
    $id=divide($_REQUEST['id']);
    $sql="SELECT concat_ws('_',id_soledad,idpeople,numsegui,evento),
      fecha_seg, numsegui, evento, estado_s, motivo_estado,
      frec_comunicacion, tiene_acompanante, tiene_contacto_emerg, participa_comunidad,
      frec_soledad, percepcion_compania, tristeza_reciente, estado_animo,
      dificultades_adl, riesgo_reciente, recibe_seguimiento,
      estrategia_1, estrategia_2, acciones_1, desc_accion1, acciones_2, desc_accion2, 
      acciones_3, desc_accion3, activa_ruta, ruta, novedades, 
      signos_covid, caso_afirmativo, otras_condiciones, observaciones,
      cierre_caso, fecha_cierre, motivo_cierre, redu_riesgo_cierre, users_bina
      FROM vsp_soledad
      WHERE id_soledad ='{$id[0]}'";
    $info=datos_mysql($sql);
    return json_encode($info['responseResult'][0]);
  } 
}

// FUNCIÓN FORMATO_DATO
function formato_dato($a,$b,$c,$d){
  $b=strtolower($b);
  $rta=$c[$d];
  if ($a=='soledad' && $b=='acciones'){
    $rta="<nav class='menu right'>";	
    $rta.="<li class='icono editar' title='Editar' id='".$c['ACCIONES']."' Onclick=\"setTimeout(getData,500,'soledad',event,this,['fecha_seg','numsegui','evento','estado_s','motivo_estado','cierre_caso'],'../vsp/soledad.php');\"></li>";
    $rta.="</nav>";
  }
  return $rta;
}

function bgcolor($a,$c,$f='c'){
  $rta="";
  return $rta;
}
?>
