...
FROM `person` P LEFT JOIN hog_tam_apgar A ON P.idpeople = A.idpeople LEFT JOIN hog_fam F ON P.vivipersona = F.id_fam LEFT JOIN hog_geo G ON F.idpre = G.idgeo
...
$sql3="SELECT  P.idpeople,-- Seguridad alimentaria (cada ítem por separado)
  ...
  CASE P.pobladifer   WHEN 1 THEN 5 WHEN 2 THEN 5 WHEN 3 THEN 5 WHEN 4 THEN 4 WHEN 5 THEN 5 WHEN 10 THEN 3 WHEN 11 THEN 4 WHEN 13 THEN 5   ELSE 0 END AS puntaje_pobladifer,
  FN_CATALOGODESC(178,P.pobladifer) AS PoblacionDirefencial,  -- Puntaje por inclusión por oficio
  FN_CATALOGODESC(179,P.incluofici) AS InclusionOficio,
  CASE P.incluofici WHEN 1 THEN 5 WHEN 3 THEN 4 WHEN 4 THEN 3 WHEN 5 THEN 5 WHEN 6 THEN 3 WHEN 7 THEN 4 WHEN 8 THEN 5  ELSE 0 END AS puntaje_incluofici, -- Puntaje bruto total vulnerabilidad social
  (CASE WHEN C.seg_pre1 = 'SI' THEN 5 ELSE 0 END +   CASE WHEN C.seg_pre2 = 'SI' THEN 5 ELSE 0 END +   CASE WHEN C.seg_pre3 = 'SI' THEN 5 ELSE 0 END +   CASE WHEN C.seg_pre4 = 'SI' THEN 5 ELSE 0 END +   CASE WHEN C.seg_pre5 = 'SI' THEN 5 ELSE 0 END +   CASE WHEN C.seg_pre6 = 'SI' THEN 5 ELSE 0 END +   CASE WHEN C.seg_pre7 = 'SI' THEN 5 ELSE 0 END +   CASE WHEN C.seg_pre8 = 'SI' THEN 5 ELSE 0 END +   CASE P.pobladifer   WHEN 1 THEN 5 WHEN 2 THEN 5 WHEN 3 THEN 5 WHEN 4 THEN 4 WHEN 5 THEN 5 WHEN 10 THEN 3 WHEN 11 THEN 4 WHEN 13 THEN 5 ELSE 0 END + CASE P.incluofici  WHEN 1 THEN 5 WHEN 3 THEN 4 WHEN 4 THEN 3 WHEN 5 THEN 5  WHEN 6 THEN 3 WHEN 7 THEN 4 WHEN 8 THEN 5 ELSE 0 END) AS puntaje_total_vulnerabilidad_social, -- Porcentaje escala 0 a 100 (máximo 50)
  ROUND(((CASE WHEN C.seg_pre1 = 'SI' THEN 5 ELSE 0 END +  CASE WHEN C.seg_pre2 = 'SI' THEN 5 ELSE 0 END +  CASE WHEN C.seg_pre3 = 'SI' THEN 5 ELSE 0 END +  CASE WHEN C.seg_pre4 = 'SI' THEN 5 ELSE 0 END +  CASE WHEN C.seg_pre5 = 'SI' THEN 5 ELSE 0 END +  CASE WHEN C.seg_pre6 = 'SI' THEN 5 ELSE 0 END +  CASE WHEN C.seg_pre7 = 'SI' THEN 5 ELSE 0 END +  CASE WHEN C.seg_pre8 = 'SI' THEN 5 ELSE 0 END + CASE P.pobladifer    WHEN 1 THEN 5 WHEN 2 THEN 5 WHEN 3 THEN 5 WHEN 4 THEN 4  WHEN 5 THEN 5 WHEN 10 THEN 3 WHEN 11 THEN 4 WHEN 13 THEN 5  ELSE 0 END + CASE P.incluofici   WHEN 1 THEN 5 WHEN 3 THEN 4 WHEN 4 THEN 3 WHEN 5 THEN 5  WHEN 6 THEN 3 WHEN 7 THEN 4 WHEN 8 THEN 5 ELSE 0 END) * 100.0 / 50), 2) AS vulnerabilidad_social_porcentaje  
FROM person P LEFT JOIN hog_fam F ON P.vivipersona = F.id_fam LEFT JOIN hog_carac C ON F.id_fam = C.idfam 
WHERE C.fecha = (SELECT MAX(C2.fecha) FROM hog_carac C2  WHERE C2.idfam = C.idfam) 
AND P.idpersona = '$document' AND P.tipo_doc = '$tipo' LIMIT 1";
...
$poblacionDif = $res3['responseResult'][0]['PoblacionDirefencial'];
$inclusionOficio = $res3['responseResult'][0]['InclusionOficio'];
...
$sql5="SELECT P.idpersona,  -- Zona y su puntaje
  FN_CATALOGODESC(3,G.zona) AS Zona, CASE WHEN G.zona = 1 THEN 1 WHEN G.zona = 2 THEN 2 ELSE NULL END AS Puntaje_Zona, -- Tipo de vivienda y puntaje
  FN_CATALOGODESC(4,C.tipo_vivienda) AS 'Tipo de Vivienda',  CASE C.tipo_vivienda
    WHEN 1 THEN 1 
    WHEN 2 THEN 1 
    WHEN 3 THEN 2
    WHEN 4 THEN 3 
...
LEFT JOIN (
  ...
  FROM hog_carac hc
  INNER JOIN (
    SELECT idfam, MAX(fecha) AS max_fecha
    FROM hog_carac
    GROUP BY idfam
  ) ult ON hc.idfam = ult.idfam AND hc.fecha = ult.max_fecha
...
$sql6="SELECT P.idpersona,-- Descripciones
  FN_CATALOGODESC(19,P.genero) AS Genero,
  P.nacionalidad AS Nacionalidad,
  FN_CATALOGODESC(16,P.etnia) AS Etnia,
  CASE P.genero WHEN 1 THEN 2 WHEN 2 THEN 1 WHEN 3 THEN 3  WHEN 4 THEN 3  WHEN 5 THEN 3 ELSE NULL END AS Puntaje_Genero,
  CASE UPPER(P.nacionalidad)  WHEN 'COL' THEN 1  WHEN 'OTR' THEN 2  WHEN 'VEN' THEN 3  ELSE NULL  END AS Puntaje_Nacionalidad,
  CASE P.etnia WHEN 1 THEN 1 WHEN 2 THEN 3 WHEN 3 THEN 3 WHEN 4 THEN 3 WHEN 5 THEN 3 WHEN 6 THEN 3 ELSE NULL END AS Puntaje_Etnia,
  -- Puntaje total bruto CD
  (CASE P.genero WHEN 1 THEN 2 WHEN 2 THEN 1 WHEN 3 THEN 3 WHEN 4 THEN 3 WHEN 5 THEN 3 ELSE 0 END + CASE UPPER(P.nacionalidad) WHEN 'COL' THEN 1 WHEN 'OTR' THEN 2 WHEN 'VEN' THEN 3 ELSE 0 END + CASE P.etnia WHEN 1 THEN 1 WHEN 2 THEN 3 WHEN 3 THEN 3 WHEN 4 THEN 3 WHEN 5 THEN 3 WHEN 6 THEN 3 ELSE 0 END) AS Puntaje_CD_Bruto,
  -- Escalado CD a 0–100
  ROUND(((CASE P.genero  WHEN 1 THEN 2 WHEN 2 THEN 1 WHEN 3 THEN 3 WHEN 4 THEN 3 WHEN 5 THEN 3 ELSE 0 END + CASE UPPER(P.nacionalidad) WHEN 'COL' THEN 1 WHEN 'OTR' THEN 2 WHEN 'VEN' THEN 3 ELSE 0 END + CASE P.etnia WHEN 1 THEN 1 WHEN 2 THEN 3 WHEN 3 THEN 3 WHEN 4 THEN 3 WHEN 5 THEN 3 WHEN 6 THEN 3 ELSE 0 END) - 3) * 100.0 / (9 - 3), 2 ) AS CD_Valor_0_100
FROM `person` P
LEFT JOIN hog_fam F ON P.vivipersona = F.id_fam
LEFT JOIN hog_geo G ON F.idpre = G.idgeo 
 WHERE P.idpersona = '$document' AND P.tipo_doc = '$tipo' LIMIT 1;";
...
$riesgos = [
    ...
    "socialVulnerability" => [
        ...
        "poblacionDiferencial" => $poblacionDif,
        "inclusionOficio" => $inclusionOficio,
...
if (!empty($document)) {
    ...
    $sqlInsert = "INSERT INTO riskfam_eval (
        idpersona, socioeconomico, estrato, ingreso, estructura_familiar, apgar, vulnerabilidad_social, poblaciondif,inclusion, acceso_salud, puntaje_regimen_salud, entorno_habitacional, puntaje_entorno, usu_create, estado
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $params = [
