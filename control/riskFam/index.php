...
<head>
  ...
  <title>Evaluación de Prioridades</title>
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
  <link href="riskFam.css?v=8.0" rel="stylesheet">
  ...
  <div class="app-container">
    <div class="app-header">
      <div class="logo-section">
        ...
        <h1>Escala de Bienestar Familiar</h1>
      </div>
      <p class="subtitle">Evaluar y Visualizar Factores de Prioridad Familiares y/o Individuales</p>
</div>
    ...
    <main class="content">
      ...
      <section class="risk-summary-section">
        <div class="card risk-summary">
          ...
          <div class="risk-gauge-container">
            ...
            <div class="risk-level low" id="riskLevel">Prioridad Baja</div>
          </div>
          ...
          <div class="risk-classification">
            ...
            <div class="risk-class high">
              <div class="risk-indicator"></div>
              <div class="risk-class-details">
                <h4>Prioridad Alto (71-100%)</h4>
                <p>La familia enfrenta múltiples factores de Prioridad y necesita intervención urgente.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section class="risk-chart-section">
        <div class="card">
          <div class="card-header">
            <h2>Visualización de Prioridades</h2>
          </div>
    ...
    <footer class="app-footer">
      <p>Herramienta de Evaluación de Prioridad Familiar &copy; 2025</p>
    </footer>
  </div>
<div id="toast-error" class="toast-error" style="display:none;"></div>
  ...
  <script>
    ...
    async function fetchDataAndUpdate() {
      ...
      if (!documentNumber) {
        showToastError(`Error: Por favor ingrese un número de documento`);
        return;
      }
      if (!tipo) {
        showToastError(`Error: Por favor seleccione el tipo de documento`);
        return;
      }
    ...
    function updateRiskFactors(riskFactors) {
      ...
      if (key === 'socioeconomic') {
        ...
        factors[key].ingreso = riskFactors[key].ingreso || 0;
          ...
          // Actualizar puntaje y apgar si existen
          if (key === 'familyStructure') {
        factors[key].puntaje = riskFactors[key].puntaje || 0;
        factors[key].apgar = riskFactors[key].apgar || 0;
        console.log('Family Structure Apgar:', factors[key].apgar); // Verifica el valor aquí
      }
      ...
      if (key === 'accessToHealth') {
        factors[key].puntajeAcceso = riskFactors[key].puntajeAcceso || 0;
        factors[key].regimen = riskFactors[key].regimen || 0;
      }
      if (key === 'livingEnvironment') {
        factors[key].zona = riskFactors[key].zona || '';
        factors[key].tipoVivienda = riskFactors[key].tipoVivienda || '';
        factors[key].tenencia = riskFactors[key].tenencia || '';
        factors[key].actividadEconomica = riskFactors[key].actividadEconomica || '';
        factors[key].energia = riskFactors[key].energia || '';
        factors[key].gas = riskFactors[key].gas || '';
        factors[key].acueducto = riskFactors[key].acueducto || '';
        factors[key].alcantarillado = riskFactors[key].alcantarillado || '';
        factors[key].basuras = riskFactors[key].basuras || '';
        factors[key].pozo = riskFactors[key].pozo || '';
        factors[key].aljibe = riskFactors[key].aljibe || '';
        factors[key]["Tráfico pesado cercano"] = riskFactors[key]["Tráfico pesado cercano"] || '';
        factors[key]["Vías sin pavimentar o en construcción cercanas"] = riskFactors[key]["Vías sin pavimentar o en construcción cercanas"] || '';
        factors[key]["Cercanía a zonas verdes y recreativas"] = riskFactors[key]["Cercanía a zonas verdes y recreativas"] || '';
        factors[key]["Cercanía a fuentes contaminantes"] = riskFactors[key]["Cercanía a fuentes contaminantes"] || '';
        factors[key]["Conserva alimentos adecuadamente"] = riskFactors[key]["Conserva alimentos adecuadamente"] || '';
        factors[key]["Manipula correctamente el agua"] = riskFactors[key]["Manipula correctamente el agua"] || '';
        factors[key]["Adquiere medicamentos con fórmula médica"] = riskFactors[key]["Adquiere medicamentos con fórmula médica"] || '';
        factors[key]["Almacena químicos de forma segura"] = riskFactors[key]["Almacena químicos de forma segura"] || '';
        factors[key]["Manejo adecuado de residuos sólidos"] = riskFactors[key]["Manejo adecuado de residuos sólidos"] || '';
      }
      if (key === 'demographics') {
            factors[key].puntaje = riskFactors[key].puntaje || 0;
    ...
    function calculateTotalRisk() {
      ...
      if (totalRisk >= 71) {
        riskLevelText = "Prioridad Alta";
        riskLevel.classList.add('high');
      } else if (totalRisk >= 41) {
        riskLevelText = "Prioridad Media";
        riskLevel.classList.add('medium');
      } else {
        riskLevelText = "Prioridad Baja";
    ...
    if (key === 'familyStructure' && (factor.puntaje || factor.apgar)) {
      ...
      const apgarText = factor.apgar ? `Descripcion Apgar: ${factor.apgar}` : '';
      additionalInfo = `
        <div class="risk-factor-tooltip">
          <div class="tooltip-content">
            ${puntajeText}<br>
    ...
    if (key === 'accessToHealth' && (factor.puntajeAcceso || factor.regimen)) {
      const puntajeText = factor.puntajeAcceso ? `Puntaje de acceso: ${factor.puntajeAcceso}` : '';
      const regimenText = factor.regimen ? `Régimen: ${factor.regimen}` : '';
      additionalInfo = `
        <div class="risk-factor-tooltip">
          <div class="tooltip-content">
            ${puntajeText}<br>
    ...
    if (key === 'livingEnvironment') {
      const zonaText = factor.zona ? `Zona: ${factor.zona}` : '';
      const tipoViviendaText = factor.tipoVivienda ? `Tipo de vivienda: ${factor.tipoVivienda}` : '';
      const tenenciaText = factor.tenencia ? `Tenencia: ${factor.tenencia}` : '';
      const energiaText = factor.energia ? `Energía: ${factor.energia}` : '';
      const gasText = factor.gas ? `Gas: ${factor.gas}` : '';
      const acueductoText = factor.acueducto ? `Acueducto: ${factor.acueducto}` : '';
      const alcantarilladoText = factor.alcantarillado ? `Alcantarillado: ${factor.alcantarillado}` : '';
      const basurasText = factor.basuras ? `Basuras: ${factor.basuras}` : '';
      const pozoText = factor.pozo ? `Pozo: ${factor.pozo}` : '';
      const aljibeText = factor.aljibe ? `Aljibe: ${factor.aljibe}` : '';
      const traficoText = factor["Tráfico pesado cercano"] ? `Tráfico pesado cercano: ${factor["Tráfico pesado cercano"]}` : '';
      const viasText = factor["Vías sin pavimentar o en construcción cercanas"] ? `Vías sin pavimentar o en construcción cercanas: ${factor["Vías sin pavimentar o en construcción cercanas"]}` : '';
      const zonasVerdesText = factor["Cercanía a zonas verdes y recreativas"] ? `Cercanía a zonas verdes y recreativas: ${factor["Cercanía a zonas verdes y recreativas"]}` : '';
      const fuentesContaminantesText = factor["Cercanía a fuentes contaminantes"] ? `Cercanía a fuentes contaminantes: ${factor["Cercanía a fuentes contaminantes"]}` : '';
      const conservaAlimentosText = factor["Conserva alimentos adecuadamente"] ? `Conserva alimentos adecuadamente: ${factor["Conserva alimentos adecuadamente"]}` : '';
      const manipulaAguaText = factor["Manipula correctamente el agua"] ? `Manipula correctamente el agua: ${factor["Manipula correctamente el agua"]}` : '';
      const adquiereMedicamentosText = factor["Adquiere medicamentos con fórmula médica"] ? `Adquiere medicamentos con fórmula médica: ${factor["Adquiere medicamentos con fórmula médica"]}` : '';
      const almacenaQuimicosText = factor["Almacena químicos de forma segura"] ? `Almacena químicos de forma segura: ${factor["Almacena químicos de forma segura"]}` : '';
      const manejoResiduosText = factor["Manejo adecuado de residuos sólidos"] ? `Manejo adecuado de residuos sólidos: ${factor["Manejo adecuado de residuos sólidos"]}` : '';
      // Puedes agregar más campos según tu JSON

      additionalInfo = `
        <div class="risk-factor-tooltip">
          <div class="tooltip-content" style="display: flex; gap: 24px;">
            <div>
              ${zonaText}<br>
              ${tipoViviendaText}<br>
              ${tenenciaText}<br>
