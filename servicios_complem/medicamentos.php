...
function cmp_medicamentctrl(){
    ...
    $c[]=new cmp('cant_entregada','nu',30,$e,$w.' '.$o,'Cantidad Entregada','cantidad_entregada',null,'',true,true,'','col-2');
    $c[]=new cmp('pend_entregar','nu',30,$e,$w.' '.$o,'Pendiente por Entregar','cantidadXaprobar',null,'',false,false,'','col-2');
    $c[]=new cmp('req_aprobacion','s',2,$e,$w.' '.$o,'Medicamento NO POS','tipo_medicamento',null,'',true,true,'','col-2',"enabPosMedicam();");
    $c[]=new cmp('cant_ordenada','nu',30,$e,$w.' POs '.$o,'Cantidad Ordenada','cant_ordenada',null,'',true,true,'','col-2');
...
$atencion=intval($_POST['cod_admision']);
  ...
  $info=datos_mysql($sql);
    ...
    if ($entregadas > 0 && $_POST['cantidad_prescrita'] != $prescrita) {
      return "msj['Error: La cantidad prescrita tiene que ser igual en las demás entregas']";
    }

// Validar que la suma de entregas no supere la prescrita
$nueva_entrega = intval($_POST['cant_entregada']);
$total_entregado = $entregadas + $nueva_entrega;
if ($total_entregado > $prescrita) {
    return "msj['Error: La cantidad total entregada no puede superar la cantidad prescrita.']";
}
  ...
  $pendiente_entregar = $prescrita-$cantidad_prescrita;
